...
def phobius_short_pred_field_selector(short_pred_line, keyword, verbose=False):
    #check_keyword = {"i":2, "o":3, "-":4, "n":4, "c":5, "s":6, "l":7}
    check_keyword = {"i":"i", "o":"o", "-":"-", "n":"-n-", "c":"c", "s":"-s-", "l":"-l-"}
    #print(keyword)
    #print(check_keyword)
    if not keyword or keyword not in check_keyword:
...
if __name__ == "__main__":
    try:
        ...
        print('Program usage: text.py <a phobius short output line> <the field keyword that has to be used to retrieve the extremes of the phobius prediction for such label> < a true or false variable to set how verbose the script should be with possible errors check the code above>', file=sys.stderr)
        raise SystemExit
    print(phobius_short_pred_field_selector(phobius_short_pred_line, field_keyword, verbose_flag))